...
export function subscriptionsFromYearlyExpenses(expectedYearlyExpenses: number): Subscription[] {
    const hfplus = halfFarePlus.map(halfFarePlus => {
        const expectedYearlyRenewals = Math.max(1, expectedYearlyExpenses / halfFarePlus.credit)
        return {
